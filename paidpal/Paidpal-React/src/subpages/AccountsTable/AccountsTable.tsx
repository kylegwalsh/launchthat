...
const GET_ACCOUNTS = gql`
  query getAccounts {
    ...
    @rest(type: "Account", path: "/get-accounts/?{args}") {
      account,
      accountId,
      platform,
      impressions,
...
export const AccountsTable = () => {
  ...
  const { data, error, loading } = useQuery(GET_ACCOUNTS, { 
    variables: { 
      filters: JSON.stringify({
        fields: Filter.formatFields(filters),
        date: {
  ...
  const columns = [
    {
      ...
      options: {
        filter: true,
        sort: true,
        display: columnDisplay.account !== undefined ? columnDisplay.account : true,
        ...
        customBodyRender: (value: any, tableMeta: any, updateRadio: any) => {
          return (
            <span>
              <Link to={`/accounts/${tableMeta.rowData[1]}/campaigns`}>{value}</Link>
            </span>
          );
        },
      },
    ...
    },
    {
      ...
      options: {
        filter: true,
        sort: true,
        display: columnDisplay.platform !== undefined ? columnDisplay.platform : true,
        customHeadRender: (columnMeta: any, handleToggleColumn: any) => (
          <CustomHeadRender
            key={columnMeta.index}
            columnMeta={columnMeta}
  ...
  return (
    <DeprecatedDataTable loading={loading} title={'Accounts'} data={data && data.accounts} options={options} columns={columns}/>
  );
};
